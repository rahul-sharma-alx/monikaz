...
import { Sparkles, Star, Clock, Shield, Search, ArrowRight, Award } from 'lucide-react';
...
export const Hero: React.FC<HeroProps> = ({
  ...
  return (
    <section id="hero-banner" className="relative bg-gradient-to-b from-[#FAF6F3] via-[#F4ECE6] to-[#FAF6F3] pt-8 pb-14 px-4 sm:px-6 lg:px-8 border-b border-[#E8DFD8] overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-10 items-center relative z-10">
        ...
        <div className="lg:col-span-7 space-y-6">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-[#EAE1D8] border border-[#D9CCC0] text-[#5C4A3E] text-xs font-semibold tracking-wide uppercase">
          ...
          <div className="pt-2 max-w-xl">
            <div className="flex flex-col sm:flex-row gap-3 p-2 bg-white rounded-2xl sm:rounded-full border border-[#E3D8CE] shadow-md">
              ...
              <button
                ...
                className="bg-[#2C221E] hover:bg-[#3D2F2A] text-white px-6 py-3 rounded-xl sm:rounded-full font-medium text-sm transition-all cursor-pointer flex items-center justify-center gap-2 shadow-sm shrink-0"
              >
                <span>Book Now</span>
          ...
          <div className="pt-2 flex flex-wrap items-center gap-2 text-xs text-[#68584E]">
            ...
            {['Hair & Styling', 'Facial & Skincare', 'Nails & Hands', 'Makeup & Bridal', 'Body Spa'].map((cat) => (
              <button
                ...
                className="px-3 py-1 rounded-full bg-white border border-[#E3D8CE] hover:border-[#A87B51] hover:text-[#A87B51] transition-all cursor-pointer shadow-2xs"
              >
                {cat}
              </button>
            ))}
        ...
        <div className="lg:col-span-5 relative">
          <div className="relative rounded-3xl overflow-hidden shadow-2xl border-4 border-white bg-stone-200">
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent flex flex-col justify-end p-6 text-white">
              <span className="text-xs uppercase tracking-widest text-[#D4AF37] font-semibold">Signature Service</span>
              <h3 className="font-serif text-xl font-bold">24K Gold Cellular Rejuvenation</h3>
              <p className="text-xs text-stone-200 mt-1">Includes LED therapy, ultrasonic cleaning & gold leaf mask.</p>
            </div>
          </div>

          {/* Floating Card Badge */}
          <div className="absolute -bottom-5 -left-5 bg-white p-4 rounded-2xl shadow-xl border border-[#E3D8CE] max-w-xs flex items-center gap-3">
